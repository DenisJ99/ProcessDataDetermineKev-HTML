...
def extract_data(text, file_type):
    ...
    # If the file type is wide, clear the running time data
    if file_type == 'wide':
        thread_running_time.clear()
    
    # If the file type is filter, clear the kernel call data
    if file_type == 'filter':
        thread_kernel_counts.clear()
        thread_cpu_events.clear()
...
def convert_kev_to_txt(kev_file, txt_file, traceprinter_path):
    command = f'"{traceprinter_path}" -f "{kev_file}" -p "t:%014t CPU:%02C %-16Z:%-18z" -o "{txt_file}"'
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error converting {kev_file} to {txt_file}: {result.stderr}")
        return False
    return True

def determine_file_type(txt_file):
    try:
        with open(txt_file, 'r') as file:
            lines = file.readlines()
            if len(lines) > 18 and '-w' in lines[18]:
                return 'wide'
            else:
                return 'filter'
    except Exception as e:
        print(f"Error determining file type for {txt_file}: {e}")
        return None

def main():
    kev_file = input("Enter the path to the KEV file: ")
    traceprinter_path = input("Enter the path to the traceprinter executable: ")
    output_file = input("Enter the name of the output HTML file: ")

    txt_file = kev_file.replace('.kev', '_converted.txt')
    if not convert_kev_to_txt(kev_file, txt_file, traceprinter_path):
        print(f"Failed to convert {kev_file} to text.")
        return

    file_type = determine_file_type(txt_file)
    if file_type is None:
        print(f"Failed to determine file type for {txt_file}.")
        return
    ...
    extracted_data = extract_data(text, file_type)
    write_to_html([extracted_data], output_file, [kev_file])
